...
interface NotasTareasDao {
    ...
    suspend fun insert(notasTareas: NotasTareas) : Long
    ...
    @Query("SELECT * from notasTareas WHERE tipo = 'note' ORDER BY fechaRegistro")
    fun getAllNotas(): Flow<List<NotasTareas>>

    //traer todas la tareas ordenas por fecha de cumplimiento
    @Query("SELECT * from notasTareas WHERE tipo = 'task' ORDER BY fechaCumplimiento")
